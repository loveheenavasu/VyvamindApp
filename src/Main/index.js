...
const DosageReminderScreen = () => {
    ...
    useEffect(() => {
        Storage.getData("ISENABLE").then((res) => {
            console.log("------res---->", res);
            if (res) {
                // setIsEnabled(true)
    ...
    useEffect(() => {
        if (isEnabled) {
            let mActor = { isEnabled: isEnabled, frequency: frequency, selectedTime: selectedTime }
    ...
    const scheduleReminder = async () => {
        const reminderOptions = {
            ...
            playSound: true,
            soundName: 'default',
            repeatType: frequency === 'daily' ? 'day' : frequency === 'weekly' ? 'week' : 'month',
            date: reminderTime,
            allowWhileIdle: true,
        ...
        <View style={styles.container}>
            ...
            <View style={styles.option}>
                ...
                <Picker
                    ...
                    style={styles.picker}
                >
                    <Picker.Item style={styles.picker_Label} label="Daily" value="daily" />
                    <Picker.Item style={styles.picker_Label} label="Weekly" value="weekly" />
                    <Picker.Item style={styles.picker_Label} label="Monthly" value="monthly" />
                </Picker>
            </View>
            <View style={styles.option}>
                <Text style={styles.label}>Reminder Time
                </Text>
                <View style={styles.time_Con} >
                    <TouchableOpacity
                        ...
                        style={{ padding: 5 }}
                    >
                        <Text style={styles.time_Label} >{selectedTime ? new Date(date).toLocaleTimeString() : "Select Time"}  </Text>
                    </TouchableOpacity>
                    <DatePicker
                        mode='time'
